...
<div class="ticcontainer" >
    ...
    $p = $con->query("SELECT * FROM flight WHERE isCompleted = 'completed' AND id IN (SELECT flightId FROM passengerflight WHERE passengerId = $userId AND status = 'registered')");
      ...
      $pep=$pe->fetch_array(MYSQLI_ASSOC);
        echo '    
  <a class="ticket" href="flight.php?current='.$flights['id'].' & companyIdd='.$flights['companyId'].'">
